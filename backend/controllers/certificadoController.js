...
exports.generarCertificado = async (req, res) => {
  try {
    ...
    // ============================================
    // DISEÑO DEL CERTIFICADO (Basado en tu PDF)
    // ============================================
    ...
    // Logo SIM (placeholder - texto por ahora)
    doc
      .fontSize(48)
      .font("Helvetica-Bold")
      .fillColor("#dc2626")
    ...
    doc
      ...
      .font("Helvetica")
      .fillColor("#374151")
      .text("Ha participado en el", doc.page.width / 2, 330, {
        align: "center",
      });
    ...
    // Tema del evento (si existe)
    if (inscripcion.evento_nombre.includes("Inteligencia")) {
      doc
        .fontSize(14)
    ...
    doc
      .fontSize(28)
      .font("Helvetica-Bold")
      .text(calidad.toUpperCase(), doc.page.width / 2, 515, {
        align: "center",
      });
...
exports.generarCertificadosMasivos = async (req, res) => {
  try {
    ...
    for (const inscripcion of inscripciones) {
      try {
        // Llamamos recursivamente a la función individual
        // (En producción, refactorizar para evitar código duplicado)
        const resultado = await generarCertificadoIndividual(
          inscripcion.id,
          inscripcion.nombre,
    ...
    res.json({
      message: `Se generaron ${certificadosGenerados.length} de ${inscripciones.length} certificados`,
...
// Función auxiliar para generación individual (reutilizable)
async function generarCertificadoIndividual(
  inscripcionId,
  nombre,
  apellido,
  calidad,
) {
  // ... (código similar al de generarCertificado, refactorizado)
...
exports.verificarEventosFinalizados = async (req, res) => {
  try {
    ...
    const [eventos] = await pool.query(
      `
      SELECT DISTINCT e.id, e.nombre, e.fecha_fin, e.hora_fin
      FROM eventos e
      INNER JOIN inscripciones i ON e.id = i.evento_id
      LEFT JOIN certificados c ON e.id = c.evento_id
      WHERE (e.fecha_fin < ? OR (e.fecha_fin = ? AND e.hora_fin < ?))
      AND c.id IS NULL
    `,
      [ahora, ahora, ahora.toTimeString().split(" ")[0]],
    );
...
exports.generarCertificadosAutomaticos = async (req, res) => {
  try {
    ...
    for (const evento of eventos) {
      // Generar certificados para todas las inscripciones de este evento
      const [inscripciones] = await pool.query(
        `
        SELECT i.id, u.nombre, u.apellido, i.calidad
        FROM inscripciones i
        INNER JOIN usuarios u ON i.usuario_id = u.id
        WHERE i.evento_id = ?
      `,
      ...
      for (const inscripcion of inscripciones) {
        try {
          // Aquí llamaríamos a la función de generación
