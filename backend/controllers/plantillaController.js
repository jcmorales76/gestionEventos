...
exports.uploadPlantilla = async function(req, res) {
  try {
    ...
    const { pos_nombre_x, pos_nombre_y, pos_tema_x, pos_tema_y, pos_calidad_x, pos_calidad_y, pos_fecha_x, pos_fecha_y, activo_nombre, activo_tema, activo_calidad, activo_fecha } = req.body;

    console.log("=== DATOS RECIBIDOS ===");
    console.log("eventoId:", eventoId);
    console.log("posiciones:", { pos_nombre_x, pos_nombre_y, pos_calidad_x, pos_calidad_y });
    console.log("activos:", { activo_nombre, activo_tema, activo_calidad, activo_fecha });
    ...
    if (existing.length > 0) {
      await pool.query("UPDATE plantillas_certificados SET url_plantilla=?, pos_nombre_x=?, pos_nombre_y=?, pos_tema_x=?, pos_tema_y=?, pos_calidad_x=?, pos_calidad_y=?, pos_fecha_x=?, pos_fecha_y=?, activo_nombre=?, activo_tema=?, activo_calidad=?, activo_fecha=? WHERE evento_id=?", [url, parseFloat(pos_nombre_x)||50, parseFloat(pos_nombre_y)||45, parseFloat(pos_tema_x)||50, parseFloat(pos_tema_y)||55, parseFloat(pos_calidad_x)||50, parseFloat(pos_calidad_y)||70, parseFloat(pos_fecha_x)||50, parseFloat(pos_fecha_y)||85, parseInt(activo_nombre)||1, parseInt(activo_tema)||0, parseInt(activo_calidad)||1, parseInt(activo_fecha)||0, eventoId]);
    } else {
...
exports.generarCertificadoConPlantilla = async function(req, res) {
  try {
    const [inscripciones] = await pool.query("SELECT i.id as inscripcion_id, i.evento_id, i.calidad, u.nombre, u.apellido FROM inscripciones i JOIN usuarios u ON i.usuario_id=u.id WHERE i.id=?", [req.params.inscripcionId]);
    ...
    const toX = function(pct) { return (parseFloat(pct)/100) * doc.page.width; };
    const toY = function(pct) { return (parseFloat(pct)/100) * doc.page.height; };

    if (parseInt(p.activo_nombre) !== 0) {
      doc.fontSize(36).font("Helvetica-Bold").fillColor("#1e3a8a").text((insc.nombre + " " + insc.apellido).toUpperCase(), toX(p.pos_nombre_x), toY(p.pos_nombre_y), { align: "center", width: 500 });
    }

    if (parseInt(p.activo_calidad) !== 0) {
      doc.fontSize(24).font("Helvetica-Bold").fillColor("#1e3a8a").text((insc.calidad || "PARTICIPANTE").toUpperCase(), toX(p.pos_calidad_x), toY(p.pos_calidad_y), { align: "center", width: 300 });
    }

    doc.end();

    stream.on("finish", async function() {
      await pool.query("INSERT INTO certificados (evento_id, inscripcion_id, tipo, nombre_participante, url_pdf) VALUES (?,?,?,?,?) ON DUPLICATE KEY UPDATE url_pdf=?", [insc.evento_id, insc.inscripcion_id, insc.calidad || "PARTICIPANTE", insc.nombre + " " + insc.apellido, urlPdf, urlPdf]);
...
exports.generarCertificadosMasivos = async function(req, res) {
  try {
    const { eventoId } = req.params;
    const [inscripciones] = await pool.query("SELECT i.id as inscripcion_id, i.evento_id, i.calidad, u.nombre, u.apellido FROM inscripciones i JOIN usuarios u ON i.usuario_id=u.id WHERE i.evento_id=?", [eventoId]);
    ...
    for (const insc of inscripciones) {
      try {
        ...
        certificadosGenerados.push({ inscripcion_id: insc.inscripcion_id, nombre: insc.nombre + " " + insc.apellido, url: urlPdf });
      } catch (error) {
        console.error("Error:", error);
      }
    }
